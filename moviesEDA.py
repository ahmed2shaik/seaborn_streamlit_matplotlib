...
@st.cache_data
def load_data():
    movie=pd.read_csv(r'D:\FSDS\Datasets\Imdb\Movie-Rating.csv')
    print("Data loaded:", movie.head())  # Print the first few rows of data
    movie.columns = ['Film', 'Genre', 'CriticsRating', 'AudienceRating', 'BudgetMillion', 'Year']
    print("Data after renaming columns:", movie.head())  # Check if column names are correct
...
st.image("D:\FSDS\Datasets\Imdb\CMovies-Logo.png")
st.title("Data analysis of movies by Waheed A Shaik")
...
st.subheader('DTypes Values')
# Capture the output of `movie.info()` into a string using StringIO
info_str = StringIO()
movie.info(buf=info_str)
info_str.seek(0)
st.text(info_str.getvalue())
# st.write(movie.info())  # .info() is not working.
# Use st.text() to display the full output
...
vis2 = sns.displot(movie.AudienceRating)
...
plt.subplot(1, 2, 1)
plt.hist(
    [movie[movie.Genre==gen].AudienceRating for gen in movie.Genre.cat.categories],
    bins=20,
...
if st.checkbox('Boxplot for Genre wise Critics Rating & Audience Ratings'):
    ...
    sns.boxplot(data=movie, x='Genre',y='AudienceRating',palette=color_dict, ax = ax[1])
    #Modifying the shape & size of x,y label and title & ticks.
...
if st.checkbox('FacetGrid for Audience Ratings vs Budget split by Genre and Year'):
    ...
    g.map(sns.scatterplot, 'AudienceRating', 'BudgetMillion', alpha=0.7)
...
if st.checkbox('Year wise boxplot of Audience rating'):
    ...
    for i, gen in enumerate(movie.Genre.cat.categories):
        ...
        sns.boxplot(data=movie[movie.Genre==gen],  x='Year',y='AudienceRating', ax=ax)
...
audience_mean = movie.groupby('Genre')['AudienceRating'].mean().sort_values(ascending=False).to_frame()  #.to_frame()--- series to dataframe.
...
k1 = sns.kdeplot(movie, y='AudienceRating', x='BudgetMillion', cmap='cool', ax=ax[0, 0])

# Plot 2: With Shade and Different Colormap
k1b = sns.kdeplot(movie, y='AudienceRating', x='BudgetMillion', cmap='magma', ax=ax[0, 0],
...
k3= sns.violinplot(movie[movie.Genre == 'Thriller'], x='Year', y='AudienceRating', ax = ax[1,0])
#Modifying the shape & size of x,y label and title & ticks.
...
k4 = sns.kdeplot(movie, x= 'CriticsRating',y='AudienceRating', \
                 shade = True,shade_lowest=False,cmap='Blues_r', \
                 ax=ax[1,1])
k4b = sns.kdeplot(movie, x= 'CriticsRating',y='AudienceRating', \
